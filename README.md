...
- **Розширений аналіз (опційно)**: Claude API (`@anthropic-ai/sdk`), викликається лише сервером
...
- **Налаштування** — керування підпискою (Stripe Billing Portal), особисті цілі, опційний ключ розширеного аналізу, вихід з акаунту.
...
src/lib/ai.ts                           — опційна інтеграція з Claude API для розширеного аналізу
src/components/Sidebar.tsx               — навігація застосунку, адаптивна (гамбургер-меню на мобільних)
```

## Розширений аналіз (опційно)

За замовчуванням поради генерує локальний аналітичний рушій (`src/lib/insights.ts`). Якщо додати ключ розширеного аналізу (через Налаштування в застосунку, або `ANTHROPIC_API_KEY` в `.env`), сервер надсилатиме агреговану статистику до Claude і повертатиме персоналізовану пораду природною мовою.
