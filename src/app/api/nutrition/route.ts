...
export async function GET() {
  ...
  const entries = await prisma.nutritionEntry.findMany({ where: { userId: auth.userId }, orderBy: { date: 'desc' } });
...
export async function DELETE(req: NextRequest) {
  ...
  await prisma.nutritionEntry.deleteMany({ where: { userId: auth.userId } });
