...
export async function GET() {
  ...
  const entries = await prisma.workoutEntry.findMany({ where: { userId: auth.userId }, orderBy: { date: 'desc' } });
...
export async function DELETE(req: NextRequest) {
  ...
  await prisma.workoutEntry.deleteMany({ where: { userId: auth.userId } });
