...
export async function GET() {
  ...
  const entries = await prisma.weightEntry.findMany({ where: { userId: auth.userId }, orderBy: { date: 'desc' } });
...
export async function DELETE(req: NextRequest) {
  ...
  await prisma.weightEntry.deleteMany({ where: { userId: auth.userId } });
